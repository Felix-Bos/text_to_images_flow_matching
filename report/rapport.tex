...
The resulting network is deliberately compact ($\approx 2$M parameters)
...
sharply within the first epoch ($1.14 \to 0.19$) and then converges
monotonically and smoothly down to $\approx 0.036$.}
\label{fig:loss}
\end{figure}
...
\toprule
Epoch & 1 & 2 & 4 & 8 & 12 & 20 & 28 & 40 \\
\midrule
Loss & 0.1910 & 0.0838 & 0.0627 & 0.0502 & 0.0453 & 0.0400 & 0.0376 & 0.0358 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{requested color} and produces an overall \textbf{silhouette}
evocative of a rotated digit, with several samples clearly recognizable as
specific digits — consistent with a $\approx 2$M parameter network trained
for 40 epochs.}
\label{fig:samples}
\end{figure}
...
\begin{itemize}
  \item \textbf{Color}: consistently respected — the easiest signal to
        ...
        epoch.
  \item \textbf{Digit shape / identity}: reasonably well learned — most
        samples show clean, continuous strokes consistent with MNIST digits,
        with several individual digits clearly identifiable; some classes
        remain ambiguous, which is expected given the model's modest
        capacity.
...
joint distribution over color, shape, and angle is well captured after a
moderate training budget on a few-million-parameter network, which validates
...
experimentation. Training converges stably (loss $1.14 \to 0.036$) and
produces samples that correctly respect the requested color while clearly
sketching the digit's shape and orientation, validating the full pipeline
from text conditioning to iterative ODE-based sampling.
